...
# Test Flow 7: Multi-API Data Aggregator
yaml_file_path = "../code_executor/sample_flow_7_api_aggregator.yaml"
print(f"Testing Flow 7: {yaml_file_path}")
...
flow_data = {
    ...
    "initial_inputs": {
        "initial_message": "Starting API aggregation test"
    }
}
...
async def test_api_aggregator():
    ...
    try:
        async with websockets.connect(websocket_url) as websocket:
            ...
            print("\n" + "="*60)
            print("FLOW 7 EXECUTION STARTED")
            print("="*60 + "\n")
            ...
            try:
                while True:
                    ...
                    if event_type == 'flow_started':
                        ...
                        # Show custom element outputs
                        if element_id == 'posts_filter':
                            post_count = outputs.get('post_count', 0)
                            print(f"   📊 Filtered {post_count} posts for user")
            ...
            print("\n" + "="*60)
            print("FLOW 7 EXECUTION SUMMARY")
            print("="*60)
...
if __name__ == "__main__":
    result = asyncio.run(test_api_aggregator())
